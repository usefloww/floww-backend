...
LABEL_IMAGE_URI = "floww.runtime.image_uri"
...
def _get_container_url(container_name: str) -> str:
    """Get the URL for a container name."""
    return f"http://{container_name}:8000"


async def _get_backend_network() -> str:
...
async def _ensure_image_exists(docker: aiodocker.Docker, image_uri: str):
    ...
    try:
        ...
        await docker.images.inspect(image_uri)
        logger.info("Docker image already exists", image_uri=image_uri)
    except aiodocker.exceptions.DockerError as e:
        if e.status == 404:
            logger.info("Pulling Docker image", image_uri=image_uri)

            # Pull the image
            await docker.images.pull(image_uri)

            logger.info("Docker image pulled successfully", image_uri=image_uri)
        else:
            raise


async def _get_config(runtime_id: str, image_uri: str, container_name: str) -> dict:
    global _backend_network
    if _backend_network is None:
        _backend_network = await _get_backend_network()
    ...
    async with httpx.AsyncClient() as client:
        try:
            response = await client.get(
                _get_container_url(container_name) + "/health",
                timeout=2.0,
            )
...
async def create_container(runtime_id: str, image_uri: str) -> None:
    ...
    async with aiodocker.Docker() as docker:
        try:
            ...
            if e.status == 404:
                ...
                await _ensure_image_exists(docker, image_uri)
                config = await _get_config(runtime_id, image_uri, container_name)
...
async def send_webhook_to_container(
    ...
    """
    container_name = _get_container_name(runtime_id)
    url = _get_container_url(container_name) + "/execute"
